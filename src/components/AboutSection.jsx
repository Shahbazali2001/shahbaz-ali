...
function StatItem({ stat, sp, triggerAt }) {
  const op = useTransform(sp, [0, triggerAt, triggerAt + 0.1, 1], [0, 0, 1, 1]);
  const y = useTransform(
    sp,
    [0, triggerAt, triggerAt + 0.1, 1],
    [12, 12, 0, 0],
  );
  return (
...
export default function AboutSection() {
  ...
  // Ambient glow
  const glowOp = useTransform(sp, [0, 0.15, 0.7, 1], [0, 0.1, 0.32, 0.32]);
        ...
        >
          ...
          >
            ...
            >
              ...
              <div
                style={{ width: 24, height: 1, background: OR, opacity: 0.5 }}
              />
            </motion.div>
          ...
          >
            ...
            >
              {LINES.map((lineWords, li) => {
                ...
                const enriched = lineWords.map((w, wi) => ({
                  ...w,
                  i: offset + wi,
                }));
                return (
                  <div key={li} style={{ display: "block" }}>
                    {enriched.map((word) => (
                      <Word key={word.i} word={word} sp={sp} />
                    ))}
                  </div>
                );
              })}
            ...
            >
              ...
              >
                ...
                products that are both functional and refined. Engineering meets
                obsessive craft.
              </p>
